...
class Converter
{
    ...
    public function convertDirectory(string $sourceDir, string $outputDir, array $variables = []): array
    {
        ...
        foreach ($files as $file) {
            if ($file->isFile() && $file->getExtension() === 'twig') {
                ...
                try {
                    if ($this->convert($file->getPathname(), $outputPath, $variables)) {
     ...
     * @param callable $filter 过滤器函数
     * @return self
     */
    public function addFilter(string $name, callable $filter): self
    {
        $this->twig->addFilter(new TwigFilter($name, $filter));
        return $this;
    }
